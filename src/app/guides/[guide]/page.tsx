...
export default function Guide() {
  ...
  useEffect(() => {
    if (params?.guide) {
      const guideFound = guideData.find((val) => val.slug === params.guide);
      if (guideFound) {
  ...
  const selectedContent = guideInfo.content[selectedCategory as keyof typeof guideInfo.content];
      ...
      <section className="mt-20 px-6 flex flex-col md:flex-row">
        ...
        <aside className="w-full md:w-1/4 bg-gray-100 p-4 rounded-md">
          ...
          <ul className="space-y-3">
            {guideInfo.categories.map((category: { id: string; title: string }) => (
              <li key={category.id}>
                <button
                  className={`text-left w-full ${
                    selectedCategory === category.id ? "text-blue-700 font-bold" : "text-blue-600"
                  } hover:underline`}
        ...
        <main className="w-full md:w-3/4 px-6">
          ...
          {selectedContent ? (
            <>
              <h2 className="text-3xl font-semibold text-gray-900">{selectedContent.title}</h2>
